...
| **toolbox** 🧰 | 高德全家桶（天气/路线/路况/POI/地理编码）+ 计算器/单位换算/时区 + 常用地点别名 | `aios toolbox {weather, route, traffic-road, poi, geo, regeo, where-add, where-list, where-rm, calc, units, tz}` | 设提醒（用 cron）/ 待办（无适配代理） |
...
> 路标：wellbeing（健康/提醒/通勤监控）、roster（人脉）—— 还没上线，遇到这些先用 `life_manager` 或自己直答兜底，并在回复里坦白"这个域还没正式子代理"。
