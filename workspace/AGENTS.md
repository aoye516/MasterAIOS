...
| **toolbox** 🧰 | 高德全家桶（天气/路线/路况/POI/地理编码）+ 计算器/单位换算/时区 + 常用地点别名 | `aios toolbox {weather, route, transit, metro-near, traffic-road, poi, geo, regeo, where-add, where-list, where-rm, calc, units, tz}` | 设提醒（用 cron）/ 待办（无适配代理） |
